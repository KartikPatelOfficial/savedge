...
import '../coupon_redemption/coupon_redemption_options_page.dart';
...
class _QRScannerPageState extends State<QRScannerPage> {
  ...
  Future<void> _handleQRCode(String qrCode) async {
    ...
    try {
      ...
      final scannedVendorName = parts.sublist(1).join('-'); // Handle vendor names with hyphens

      // Validate vendor matches
      if (scannedVendorId != widget.expectedVendorId || 
          scannedVendorName.toLowerCase() != widget.expectedVendorName.toLowerCase()) {
      ...
      if (!couponCheck.canUserRedeem) {
        throw Exception('Coupon cannot be redeemed: ${couponCheck.redeemabilityReasons.join(', ')}');
      }
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              Navigator.of(context).pop(false); // Return to previous page with failure
            },
            child: const Text('Cancel'),
          ),
        ],
