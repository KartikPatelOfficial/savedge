...
import '../../features/auth/domain/entities/extended_user_profile.dart';
import '../../features/auth/domain/repositories/auth_repository.dart';
import '../../features/subscription/domain/services/razorpay_payment_service.dart';
...
class SubscriptionPurchasePage extends StatefulWidget {
  ...
  @override
  State<SubscriptionPurchasePage> createState() => _SubscriptionPurchasePageState();
}
...
class _SubscriptionPurchasePageState extends State<SubscriptionPurchasePage> {
  ...
  RazorpayPaymentService get _razorpayService => GetIt.I<RazorpayPaymentService>();
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          if (profile.isEmployee && profile.pointsBalance >= _calculatePointsCost()) {
    ...
    return Container(
      ...
      child: plan.hasImage 
          ? Image.network(
              plan.imageUrl!, 
              fit: BoxFit.cover,
              errorBuilder: (context, error, stackTrace) => _buildGradientHeader(),
            )
  ...
  Widget _buildPointsBalance() {
    ...
    final hasEnoughPoints = _userProfile!.pointsBalance >= _calculatePointsCost();
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 14,
                        color: hasEnoughPoints ? Colors.green[600] : Colors.red[600],
                        fontWeight: FontWeight.w500,
                      ),
                    ),
                  ],
  ...
  Widget _buildPurchaseButton() {
    ...
    final canPurchase = !isPointsPayment || 
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          onPressed: canPurchase && !_isProcessingPayment ? _handlePurchase : null,
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text('Balance after purchase: ${(_userProfile?.pointsBalance ?? 0) - _calculatePointsCost()}'),
          ],
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      if (mounted) {
        ...
        if (result['success'] == true) {
          _showSuccessDialog('Subscription purchased successfully with points!');
        } else {
          _showErrorDialog(result['message'] ?? 'Failed to purchase subscription');
        }
      }
    } catch (e) {
      if (mounted) {
